...
fn crew_label(key: &str) -> &str {
    match key {
        ...
        "uav_name" => "Назва БпАК",
        "uav_type" => "Тип БпАК",
        "asset_kind" => "Вид об’єкта (aircraft/complex)",
        "components_json" => "Комплектація БпАК",
        "assigned_quantity" => "Закріплена кількість",
...
fn equipment_label(key: &str) -> &str {
    match key {
        ...
        "uav_type" => "Тип БпАК",
        "service_code" => "Код служби",
        "catalog_name" => "Каталог",
        "full_name" => "Найменування матеріальних засобів",
        "nomenclature_number" => "Номенклатурний номер",
        "serial_number" => "Серійний / заводський номер",
        "manufacture_year" => "Рік виготовлення",
        "accounting_unit" => "Од. обліку",
        "quantity" => "Кількість",
        "asset_value" => "Вартість",
        "parent_inventory_number" => "Належить до (інвентарний номер)",
        "asset_type" => "Тип об’єкта",
        "service_data_json" => "Службові поля (JSON)",
        "catalog_fields_json" => "Опис додаткових полів каталогу (JSON)",
        "custom_values_json" => "Додаткові поля каталогу (JSON)",
        "notes" => "Примітка",
        _ => key,
    }
}
        ...
        equipment
            ...
            .map(|row| {
                vec![
                    ...
                    row.uav_type.clone(),
                    row.asset_kind.clone(),
                    row.components_json.clone(),
                    row.assigned_quantity.clone(),
                    row.service_code.clone(),
                    row.catalog_name.clone(),
                    row.full_name.clone(),
                    row.nomenclature_number.clone(),
                    row.serial_number.clone(),
                    row.manufacture_year.clone(),
                    row.accounting_unit.clone(),
                    row.quantity.clone(),
                    row.asset_value.clone(),
                    row.parent_inventory_number.clone(),
                    row.asset_type.clone(),
                    row.service_data_json.clone(),
                    row.catalog_fields_json.clone(),
                    row.custom_values_json.clone(),
                    row.notes.clone(),
                ]
            })
            .collect::<Vec<_>>()
