...
export function ReportGenerationPage({ template, templates, hasMoreTemplates, isLoadingMoreTemplates, onLoadMoreTemplates, people, hasMorePeople, isLoadingMorePeople, onLoadMorePeople, selected, onToggle, onClear, onReorder, onChoose }: Props) {
  ...
  const moveRequirement = (requirement: SelectionRequirement, id: number, direction: -1 | 1) => {
    if (requirement.id === "personnel") onReorder?.(moved(selected, id, direction));
      ...
      const categoryIds = new Set(equipment.filter((item) => item.category === requirement.category).map((item) => item.id));
