    ...
    {parameters.map(({ token, field }) => {
      ...
      const control = field.inputType === "textarea"
        ...
        : field.inputType === "boolean"
          ? <select aria-label={name} value={values[token] ?? "Ні"} onChange={(event) => onChange(token, event.target.value)}><option value="Так">Так</option><option value="Ні">Ні</option></select>
          : <input aria-label={name} type={field.inputType} value={values[token] ?? ""} onChange={(event) => onChange(token, event.target.value)} />;
      return <label className={`generation-parameter generation-parameter--${field.inputType}`} key={token}><span><b>{name}</b><code>{`{{${token}}}`}</code></span>{control}</label>;
    })}
  </div>;
}
...
export function ReportGenerationPage({ template, templates, hasMoreTemplates, isLoadingMoreTemplates, onLoadMoreTemplates, people, hasMorePeople, isLoadingMorePeople, onLoadMorePeople, selected, onToggle, onAll, onClear, onChoose }: Props) {
  ...
  return <PageFrame className="generation-page"><div className="generation-layout">
    ...
    <section className="selection-column">{!template?.sourcePath ? <div className="panel people-select generation-empty-state"><FileText /><h2>Оберіть шаблон</h2><p>Виберіть DOCX зі списку ліворуч — тут з’являться потрібні поля та вибір даних.</p></div> : !needsSelection ? <div className="panel people-select generation-parameters-only"><h2>Параметри значень</h2><p>{parameters.length ? "Заповніть лише значення, потрібні для цього рапорту. Номер після назви означає окреме значення." : "Для цього шаблону не потрібно обирати дані або заповнювати додаткові значення."}</p>{parameterFields}</div> : vehicleOnly ? <div className="panel people-select"><h2>Вибір автомобілів</h2><div className="people-select__scroll"><table><thead><tr><th></th><th>Автомобіль</th><th>Номер</th><th>Стан</th><th>Водій</th></tr></thead><tbody>{vehicles.map((vehicle) => <tr key={vehicle.id} onClick={() => toggleVehicle(vehicle.id)} className={selectedVehicles.includes(vehicle.id) ? "selected-row" : ""}><td><CheckBox checked={selectedVehicles.includes(vehicle.id)} onChange={() => toggleVehicle(vehicle.id)} /></td><td>{vehicle.name}</td><td>{vehicle.registrationNumber}</td><td>{vehicle.status}</td><td>{vehicle.driverName ?? "—"}</td></tr>)}</tbody></table></div>{parameterFields}<div className="selection-footer">Вибрано: <b className="green">{selectedVehicles.length}</b><span /><button className="button" onClick={() => setSelectedVehicles([])} disabled={!selectedVehicles.length}>Очистити вибір</button><button className="button" onClick={() => setSelectedVehicles(vehicles.map((vehicle) => vehicle.id))}>Вибрати всі</button></div></div> : <div className="panel people-select"><h2>Вибір військовослужбовців</h2><div className="table-tools"><SearchInput placeholder="Пошук за ПІБ, ІПН, посадою…" value={personQuery} onChange={setPersonQuery} /></div><div className="people-select__scroll" onScroll={onPeopleScroll}><table><thead><tr><th><CheckBox checked={selected.length === people.length} onChange={onAll} /></th><th>Звання</th><th>ПІБ</th><th>Посада</th></tr></thead><tbody>{filteredPeople.map((person) => <tr key={person.id} onClick={() => onToggle(person.id)} className={selected.includes(person.id) ? "selected-row" : ""}><td><CheckBox checked={selected.includes(person.id)} onChange={() => onToggle(person.id)} /></td><td>{person.rank}</td><td>{person.fullName}</td><td>{person.position}</td></tr>)}</tbody></table>{isLoadingMorePeople && <div className="infinite-loading">Завантаження наступних 20 осіб…</div>}</div>{parameterFields}<div className="selection-footer">Вибрано: <b className="green">{selected.length}</b><span /><button className="button" onClick={onClear} disabled={!selected.length}>Очистити вибір</button><button className="button" onClick={onAll}>Вибрати всіх</button></div></div>}<button className="generate-button" disabled={!canGenerate || isGenerating} onClick={generateReport}><FileCheck2 />{isGenerating ? "Створення рапорту…" : "Згенерувати рапорт"}</button>{!canGenerate && <p className="generate-hint">Оберіть шаблон{template?.sourcePath && needsSelection ? ` та ${vehicleOnly ? "автомобіль" : "військовослужбовців"}` : ""}{template?.sourcePath && parameters.length ? " і заповніть параметри значень" : ""}.</p>}{generatedReport && <div className="generation-result"><div><button className="button primary" onClick={() => void openReport(generatedReport.docxPath)}><FileText />Відкрити DOCX</button><button className="button" onClick={() => void openReportFolder(generatedReport.folderPath)}><FolderOpen />Відкрити папку</button></div></div>}</section>
  </div></PageFrame>;
}
