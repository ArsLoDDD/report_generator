...
export function ReportGenerationPage({ template, templates, hasMoreTemplates, isLoadingMoreTemplates, onLoadMoreTemplates, people, hasMorePeople, isLoadingMorePeople, onLoadMorePeople, selected, onToggle, onClear, onChoose }: Props) {
  ...
  const selectedFor = (requirement: SelectionRequirement) => requirement.id === "personnel" ? selected : requirement.id === "vehicle" ? selectedVehicles : requirement.id === "crew" ? selectedCrews : selectedEquipment.filter((id) => equipment.some((item) => item.id === id && item.category === requirement.category));
  ...
  const generateReport = () => { if (!template?.sourcePath) return; const categories = new Set(requirements.flatMap((item) => item.category ? [item.category] : [])); const equipmentIds = selectedEquipment.filter((id) => equipment.some((item) => item.id === id && categories.has(item.category))); generate(template.sourcePath, requirements.some((item) => item.id === "personnel") ? selected : [], parameterValues, requirements.some((item) => item.id === "vehicle") ? selectedVehicles : [], requirements.some((item) => item.id === "crew") ? selectedCrews : [], equipmentIds); };
  ...
  const requirementColumns = (requirement: SelectionRequirement) => requirement.id === "personnel" ? ["Звання", "ПІБ", "Посада"] : requirement.id === "vehicle" ? ["Автомобіль", "Номер", "Стан", "Водій"] : requirement.id === "crew" ? ["Екіпаж", "Взвод", "Позиція", "Склад"] : ["Назва", "Інвентарний номер", "Стан", "Закріплено за"];
