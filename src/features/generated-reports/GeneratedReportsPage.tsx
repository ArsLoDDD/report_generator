...
export function GeneratedReportsPage() {
  ...
  return <PageFrame header={<PageTitle title="Згенеровані рапорти" subtitle="Файли, створені у папці Reports" actions={<><button className="button" onClick={() => void refresh()}><RefreshCw />Оновити</button><button className="button"><Download />Експорт списку</button><button className="button primary" onClick={() => details && void generatedReportsService.openFolder(details.folderPath)} disabled={!details}><FolderOpen />Відкрити папку з рапортом</button></>} />} tools={tools} footer={<div className="statbar"><Stat icon={Archive} label="Усього рапортів" value={reports.length} /><Stat icon={CalendarDays} label="За результатами пошуку" value={filteredReports.length} /><Stat icon={FileText} label="Джерело" value="Reports" /></div>} className="generated-page"><div className="generated-layout"><section className="panel data-table"><div className="data-table__scroll"><table><thead><tr><th><CheckBox /></th><th>Назва рапорту</th><th>Шаблон</th><th>Дата генерації</th><th>Дії</th></tr></thead><tbody>{filteredReports.map((report) => <tr className={details?.folderPath === report.folderPath ? "selected-row" : ""} key={report.folderPath} onClick={() => setSelectedReport(report)}><td><CheckBox checked={details?.folderPath === report.folderPath} onChange={() => setSelectedReport(report)} /></td><td><span className="word-icon">W</span>{report.name}</td><td>{report.template}</td><td>{report.generatedAt}</td><td><button className="icon-button" aria-label={`Відкрити ${report.name}`} onClick={(event) => { event.stopPropagation(); void generatedReportsService.openDocument(report.docxPath); }}><Eye /></button><button className="icon-button" aria-label={`Відкрити папку ${report.name}`} onClick={(event) => { event.stopPropagation(); void generatedReportsService.openFolder(report.folderPath); }}><FolderOpen /></button><button className="icon-button"><MoreVertical /></button></td></tr>)}</tbody></table></div><div className="pagination">{isLoading ? "Завантаження…" : `Показано ${filteredReports.length} із ${reports.length}`}</div>{errorMessage && <p className="generation-message error">{errorMessage}</p>}</section><aside className="panel report-details">{details && <button className="close" onClick={() => setSelectedReport(null)}><X /></button>}<h2>{details?.name ?? "Рапортів не знайдено"}</h2>{details ? <><p><span className="word-icon">W</span> DOCX · {details.generatedAt}</p><div className="actions-line"><button className="button primary" onClick={() => void generatedReportsService.openDocument(details.docxPath)}><Eye />Відкрити</button><button className="button" onClick={() => void generatedReportsService.openFolder(details.folderPath)}><FolderOpen />Відкрити папку</button></div></> : <p>Після створення рапорти з’являться тут автоматично.</p>}</aside></div></PageFrame>;
}
