...
function SignersGuide() {
  return <section className="documentation-flow"><header><PenTool /><div><h2>Підписанти та підпис</h2><p>Налаштовуються у розділі «Налаштування» та застосовуються до кожної генерації.</p></div></header><ol><li><b>Основний підписант</b> використовує простір імен <code>main</code>: звання, окремі частини ПІБ, повне ПІБ і посаду. Лише він має <code>{"{{main.signature}}"}</code> — PNG-підпис із папки «Підписи».</li><li><b>Командир</b> використовує простір імен <code>commander</code>: <code>{"{{commander.rank}}"}</code>, <code>{"{{commander.surname}}"}</code>, <code>{"{{commander.givenName}}"}</code>, <code>{"{{commander.patronymic}}"}</code>, <code>{"{{commander.fullName}}"}</code> та <code>{"{{commander.position}}"}</code>.</li><li><b>Начальник штабу</b> використовує такий самий набір у просторі <code>chief</code>: <code>{"{{chief.rank}}"}</code>, <code>{"{{chief.surname}}"}</code>, <code>{"{{chief.givenName}}"}</code>, <code>{"{{chief.patronymic}}"}</code>, <code>{"{{chief.fullName}}"}</code> та <code>{"{{chief.position}}"}</code>. Для командира й начальника штабу графічний підпис не використовується.</li><li>PNG-файл підпису за замовчуванням має назву <code>main.png</code>; її можна змінити в налаштуваннях без перейменування самого файлу.</li><li>Папки даних створюються програмою автоматично, тому шляхи вручну не налаштовуються.</li></ol></section>;
}
