...
import { modifierRegistry, signerFields, signerRoles, tokenFor, variableRegistry, vehicleFields, type VariableDefinition } from "../../shared/template-language/registry";
...
export function VariableConstructorPage() {
  ...
  const objects = [{ id: "person", label: "Військовослужбовець" }, { id: "vehicle", label: "Автомобіль" }, ...signerObjects.map(([id, label]) => ({ id, label })), { id: "document", label: "Параметри документа" }];
  ...
  const variableId = viewMode === "all" ? field.id : isPerson ? field.id.replace(/^військовий_1_/, `військовий_${Math.max(1, Number(personNumber) || 1)}_`) : objectId === "document" && Number(parameterNumber) > 0 ? `${field.id}_${Math.floor(Number(parameterNumber))}` : field.id;
  const token = tokenFor(variableId, modifiers);
  const selectObject = (id: string) => { setObjectId(id); setFieldId(id === "person" ? "військовий_1_піб" : id === "vehicle" ? "автомобіль_назва" : id === "document" ? "дата_рапорту" : `${id}_піб`); setParameterNumber(""); setStep(1); setModifiers([]); };
