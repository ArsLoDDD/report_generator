...
import type { Crew, CrewDraft, Equipment, Incident, Position } from "./types";
...
export function CrewsPage({ people }: { people: Person[] }) {
  ...
  const edit = (crew?: Crew) => { setEditing(crew ?? null); setDraft(crew ? { ...crew, memberIds: crew.members.map((member) => member.personnelId), actualMemberIds: crew.actualMembers.map((member) => member.personnelId) } : emptyDraft()); setEditorTab("overview"); setOpen(true); };
  ...
  return <PageFrame className="crews-page" header={<PageTitle title="Екіпажі" subtitle="Єдиний облік екіпажів, позицій, складу та майна" actions={<button className="button primary" onClick={() => edit()}><Plus />Створити екіпаж</button>} />} tools={<RegistryToolbar placeholder="Пошук за назвою, статусом, смугою, БпАК або позицією…" query={query} onQueryChange={setQuery} resultCount={filtered.length} />}>
    ...
    {open && <Modal title={editing ? editing.name : "Новий екіпаж"} subtitle={editing ? `${editing.sector || "Смуга не вказана"} · ${editing.positionName || "позиція не обрана"}` : undefined} onClose={close} className="crew-editor"><div className="crew-editor__body">
      ...
      {editorTab === "assets" && <section className="crew-editor__uavs"><div className="crew-members-heading"><p><b>Майно екіпажу</b><br />Показане пряме майно екіпажу та майно, закріплене за його людьми.</p><button className="button" onClick={() => setUavPickerOpen(true)}><Plus />Закріпити БпЛА</button></div><div className="crew-asset-groups">{crewAssets.length > 0 && <section><h3>БпЛА та БпАК</h3><div className="crew-selected-members">{crewAssets.map((uav) => <article key={uav.id}><div><b>{uav.name}</b><small>{uav.inventoryNumber || "Без номера"} · {uav.assignedQuantity || uav.totalQuantity} шт · денні {uav.dayQuantity}, нічні {uav.nightQuantity}</small></div>{uav.id === draft.primaryUavId && <span className="crew-asset-primary">Основний</span>}<button className="icon-button danger" title="Зняти з екіпажу" onClick={() => void assignUav(uav, null)}><Trash2 /></button></article>)}</div></section>}{crewVehicles.length > 0 && <section><h3>Автомобілі</h3><div className="crew-selected-members">{crewVehicles.map((vehicle) => <article key={vehicle.id}><div><b>{vehicle.name}</b><small>{vehicle.registrationNumber} · {vehicle.status}{vehicle.driverName ? ` · ${vehicle.driverName}` : ""}</small></div><button className="icon-button danger" title="Зняти автомобіль з екіпажу" onClick={() => void unassignVehicle(vehicle)}><Trash2 /></button></article>)}</div></section>}{crewOtherAssets.length > 0 && <section><h3>Інше майно</h3><div className="crew-selected-members">{crewOtherAssets.map((asset) => <article key={asset.id}><div><b>{asset.name}</b><small>{asset.inventoryNumber || "Без номера"} · {asset.totalQuantity} шт · {asset.holderName || asset.crewName || "без відповідального"}</small></div>{asset.crewId === editing?.id && <button className="icon-button danger" title="Зняти майно з екіпажу" onClick={() => void assignUav(asset, null)}><Trash2 /></button>}</article>)}</div></section>}{crewAssetCount === 0 && <div className="crew-assets-empty"><PackageOpen /><span>За екіпажем і його особовим складом майно не закріплене.</span></div>}</div></section>}
