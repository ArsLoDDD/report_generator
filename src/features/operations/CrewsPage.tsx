...
type PendingMemberRemoval = { id: number; fullName: string; memberTab: "official" | "actual" };
type PendingAssetRemoval = { kind: "equipment"; item: Equipment } | { kind: "vehicle"; item: Vehicle };
...
export function CrewsPage({ people }: { people: Person[] }) {
  ...
  const [pendingMemberRemoval, setPendingMemberRemoval] = useState<PendingMemberRemoval | null>(null);
  ...
  const reload = useCallback(async () => {
    setIsLoading(true);
    try {
      const [crews, nextPositions, nextPeople, nextUavs, nextIncidents, nextEquipment, nextVehicles] = await Promise.all([operationsService.listCrews(), operationsService.listPositions(), allPersonnel(), operationsService.listEquipment("uav"), operationsService.listIncidents(), Promise.all([operationsService.listEquipment("generator"), operationsService.listEquipment("communications"), operationsService.listEquipment("weapon_ammo")]), vehiclesService.list()]);
      setItems(crews); setPositions(nextPositions); setAvailablePeople(nextPeople); setUavs(nextUavs ?? []); setIncidents(nextIncidents ?? []); setOtherEquipment(nextEquipment.flatMap((group) => group ?? [])); setVehicles(nextVehicles ?? []);
    } catch {
      notify("Не вдалося завантажити екіпажі.", "error");
    } finally {
      setIsLoading(false);
    }
  }, [notify]);
  ...
  const close = () => { setOpen(false); setEditing(null); setDraft(emptyDraft()); setEditorTab("overview"); setMemberTab("official"); setPickerOpen(false); setPendingMemberRemoval(null); setPendingAssetRemoval(null); setSaving(false); savingRef.current = false; };
  const edit = (crew?: Crew) => { const next = crew ? { ...crew, memberIds: crew.members.map((member) => member.personnelId), actualMemberIds: crew.actualMembers.map((member) => member.personnelId) } : emptyDraft(); setEditing(crew ?? null); setDraft(next); initialDraftRef.current = JSON.stringify(next); setEditorTab("overview"); setAssetTab("uav"); setOpen(true); };
  const persistDraft = async (closeAfterSave: boolean, nextDraft = draft) => {
    if (savingRef.current) return false;
    if (!nextDraft.name.trim()) { notify("Вкажіть назву екіпажу.", "error"); return false; }
    savingRef.current = true; setSaving(true);
    try {
      const payload = { ...nextDraft, officialStrength: nextDraft.memberIds.length, workingStrength: nextDraft.actualMemberIds.length };
      if (editing) await operationsService.updateCrew(editing.id, payload); else await operationsService.createCrew(payload);
      initialDraftRef.current = JSON.stringify(nextDraft);
      if (closeAfterSave) close();
      await reload();
      notify(closeAfterSave ? "Екіпаж збережено." : "Зміни складу екіпажу збережено.", "success");
      return true;
    } catch (error) {
      notify(typeof error === "string" ? error : "Не вдалося зберегти екіпаж.", "error"); return false;
    } finally {
      savingRef.current = false; setSaving(false);
    }
  };
  const save = () => persistDraft(true);
  const closeAndSave = () => {
    if (savingRef.current) return;
    if (JSON.stringify(draft) === initialDraftRef.current || (!editing && !draft.name.trim())) { close(); return; }
    void save();
  };
  ...
  const toggleInDraft = (current: CrewDraft, id: number, targetTab: "official" | "actual") => {
    if (targetTab === "actual") return { ...current, actualMemberIds: current.actualMemberIds.includes(id) ? current.actualMemberIds.filter((value) => value !== id) : [...current.actualMemberIds, id] };
  ...
  const toggle = (id: number) => {
    if (memberIds.includes(id)) {
      const person = availablePeople.find((item) => item.id === id);
      setPendingMemberRemoval({ id, fullName: person?.fullName || `військовослужбовець №${id}`, memberTab });
      return;
    }
    setDraft((current) => toggleInDraft(current, id, memberTab));
  };
  const confirmMemberRemoval = async () => {
    if (!pendingMemberRemoval) return;
    const removal = pendingMemberRemoval;
    const nextDraft = toggleInDraft(draft, removal.id, removal.memberTab);
    setDraft(nextDraft);
    setPendingMemberRemoval(null);
    if (editing && !pickerOpen) await persistDraft(false, nextDraft);
  };
  const closeMemberPicker = async () => {
    if (savingRef.current) return;
    if (editing && JSON.stringify(draft) !== initialDraftRef.current) {
  ...
  const confirmAssetRemoval = async () => {
    ...
    const removal = pendingAssetRemoval;
    if (removal.kind === "vehicle") await unassignVehicle(removal.item);
    else await assignUav(removal.item, null);
  ...
  const availableCrewPositions = positions.filter((position) => position.positionType !== "Облаштовується");
  const blockedDraftPosition = positions.find((position) => position.id === draft.positionId && position.positionType === "Облаштовується");
  ...
  return <PageFrame className="crews-page" onContentScroll={onContentScroll} footer={<div className="panel pagination card-registry__pagination">Показано {visibleItems.length} із {filtered.length}</div>} header={<PageTitle title="Екіпажі" subtitle="Єдиний облік екіпажів, позицій, складу та майна" actions={<button className="button primary" onClick={() => edit()}><Plus />Створити екіпаж</button>} />} tools={<RegistryToolbar className="card-registry-toolbar" placeholder="Пошук за назвою, статусом, смугою, БпАК або позицією…" query={query} onQueryChange={setQuery} />}>
    ...
    {pendingMemberRemoval && <ConfirmDialog title="Прибрати військовослужбовця зі складу?" message={`«${pendingMemberRemoval.fullName}» буде вилучено з ${pendingMemberRemoval.memberTab === "official" ? "офіційного" : "фактичного"} складу екіпажу. Запис військовослужбовця з бази не видаляється.`} confirmLabel="Прибрати зі складу" onConfirm={() => { void confirmMemberRemoval(); }} onCancel={() => setPendingMemberRemoval(null)} busy={saving} />}
    {pendingAssetRemoval && <ConfirmDialog title="Зняти майно з екіпажу?" message={`«${pendingAssetRemoval.item.name}» буде відв’язано від екіпажу. Сам запис майна з обліку не видаляється.`} confirmLabel="Зняти з екіпажу" onConfirm={() => { void confirmAssetRemoval(); }} onCancel={() => setPendingAssetRemoval(null)} />}
