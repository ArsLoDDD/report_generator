...
export function CrewsPage({ people }: { people: Person[] }) {
  ...
  const close = () => { setOpen(false); setEditing(null); setDraft(emptyDraft()); setEditorTab("overview"); setMemberTab("official"); setPickerOpen(false); setSaving(false); savingRef.current = false; };
  ...
  const save = async () => {
    ...
    if (!draft.name.trim()) { notify("Вкажіть назву екіпажу.", "error"); return false; }
    savingRef.current = true; setSaving(true);
    try {
      const payload = { ...draft, officialStrength: draft.memberIds.length, workingStrength: draft.actualMemberIds.length };
      if (editing) await operationsService.updateCrew(editing.id, payload); else await operationsService.createCrew(payload);
      close(); reload(); notify("Екіпаж збережено.", "success"); return true;
    } catch (error) {
