...
function DutyEditor({ title, items, people, reportDate = initialReportDate(), onChange }: { title: string; items: SummaryDutyItem[]; people: StaffingRecord[]; reportDate?: string; onChange: (items: SummaryDutyItem[]) => void }) {
  ...
  return <section className="summary-editor-block summary-duty-editor"><header><div><b>{title}</b><small>Графік має безперервно покривати всю добу 18:01–18:00. Кожен період можна скоригувати або видалити окремо.</small></div><div className="summary-header-actions"><span className="source-badge">Вручну</span><Select ariaLabel="Тривалість однієї зміни" value={shiftHours} onChange={setShiftHours} options={[1, 2, 3, 4, 6, 8, 12].map((hours) => ({ value: String(hours), label: `${hours} год` }))} /><button className="button compact" disabled={!items.some((item) => item.personnelId)} onClick={() => onChange(buildAlternatingDutySchedule(items, reportDate, Number(shiftHours)))}><Clock3 />Розподілити добу</button><button className="icon-button" aria-label={`Додати: ${title}`} onClick={() => onChange([...items, { id: crypto.randomUUID(), personnelId: null, periods: [] }])}><Plus /></button></div></header>{items.length ? <div className="summary-duty-list">{items.map((item) => <article className="summary-duty-object" key={item.id}><header><Select ariaLabel="Військовослужбовець" value={String(item.personnelId ?? "")} onChange={(value) => setPerson(item.id, value ? Number(value) : null)} options={[{ value: "", label: "Оберіть військовослужбовця" }, ...available.filter((person) => person.personnelId === item.personnelId || !selectedIds.has(person.personnelId)).map((person) => ({ value: String(person.personnelId), label: `${person.fullName} · ${person.rank}` }))]} /><div className="summary-card-actions"><button className="button compact" onClick={() => addPeriod(item.id)}><Plus />Додати період</button><button className="icon-button danger" aria-label="Видалити військовослужбовця" onClick={() => onChange(items.filter((value) => value.id !== item.id))}><Trash2 /></button></div></header><div className="summary-duty-period-list">{item.periods.map((period) => <div className="summary-duty-period" key={period.id}><div><input aria-label="Дата першого часу" type="date" value={period.startDate} onChange={(event) => setPeriod(item.id, period.id, { startDate: event.target.value })} /><input aria-label="Перший час" type="time" value={period.startTime} onChange={(event) => setPeriod(item.id, period.id, { startTime: event.target.value })} /></div><span>–</span><div><input aria-label="Дата другого часу" type="date" value={period.endDate} onChange={(event) => setPeriod(item.id, period.id, { endDate: event.target.value })} /><input aria-label="Другий час" type="time" value={period.endTime} onChange={(event) => setPeriod(item.id, period.id, { endTime: event.target.value })} /></div><button className="icon-button danger" aria-label="Видалити період" onClick={() => removePeriod(item.id, period.id)}><Trash2 /></button></div>)}{!item.periods.length && <small>Періодів ще немає. Розподіліть добу автоматично або додайте період.</small>}</div></article>)}</div> : <small>Чергових ще не додано.</small>}</section>;
}
...
function FlightItemsEditor({ automatic, value, onChange }: { automatic: SummaryFlightItem[]; value: SummaryFlightItem[] | null; onChange: (items: SummaryFlightItem[] | null) => void }) {
  ...
  return <section className="summary-editor-block summary-flight-editor">
    ...
    {editing && <Modal title={`Екіпаж: ${editing.crewName}`} subtitle="Позиція та смуга підтягуються зі знімка плану польотів і не змінюються тут." onClose={() => setEditing(null)} className="summary-flight-modal"><div className="summary-flight-modal__body">
      ...
      <section className="summary-modal-group"><header><div><b>Фактичний склад на позиції</b><small>ПІБ береться з етапу плану; редагується лише період.</small></div></header>{editing.members.map((member) => <article className="summary-member-row" key={member.id}><div><b>{member.fullName}</b><small>{member.rank}</small></div><div className="summary-member-times"><input type="date" value={member.startDate} onChange={(event) => setEditing({ ...editing, members: editing.members.map((item) => item.id === member.id ? { ...item, startDate: event.target.value } : item) })} /><input type="time" value={member.startTime} onChange={(event) => setEditing({ ...editing, members: editing.members.map((item) => item.id === member.id ? { ...item, startTime: event.target.value } : item) })} /><input type="date" value={member.endDate} onChange={(event) => setEditing({ ...editing, members: editing.members.map((item) => item.id === member.id ? { ...item, endDate: event.target.value } : item) })} /><input type="time" value={member.endTime} onChange={(event) => setEditing({ ...editing, members: editing.members.map((item) => item.id === member.id ? { ...item, endTime: event.target.value } : item) })} /></div></article>)}</section>
