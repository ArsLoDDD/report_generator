...
export function SummaryReportPage() {
  ...
  const loadSources = useCallback(async (date: string) => {
    ...
    const previousSnapshot = flightPlanPendingDraftRequest(previousDate) ?? parseSnapshot(previousStored) ?? flightPlanDraftRequest(previousDate);
    const currentSnapshot = flightPlanPendingDraftRequest(date) ?? parseSnapshot(currentStored) ?? flightPlanDraftRequest(date);
