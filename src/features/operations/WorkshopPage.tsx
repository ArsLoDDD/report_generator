...
export function WorkshopPage({ embedded = false, toolbarLeading }: { embedded?: boolean; toolbarLeading?: ReactNode }) {
  ...
  return <PageFrame className={`workshop-page ${embedded ? "workshop-page--embedded" : ""}`} header={embedded
    ...
    <section className="panel operation-table data-table workshop-table"><EntityTable items={products} columns={columns} rowKey={(item) => item.id} emptyState={<div className="personnel-state"><ServiceIcon name="workshop" /><b>Виробів ще немає</b><span>Перше виготовлення з’явиться тут після облікової операції.</span></div>} /><div className="pagination">Показано {products.length} із {products.length}</div></section>
    ...
    {historyOpen && <Modal title="Історія виготовлень" subtitle="Усі обліковані вироби та фактично списані складові" onClose={() => setHistoryOpen(false)} className="workshop-history-modal"><div className="workshop-history-list">{products.length ? products.map((product) => { const occurred = incidentDateTimeParts(product.createdAt); return <article key={product.id}><span className="workshop-history-list__icon"><ServiceIcon name="workshop" /></span><div><header><b>{product.name}</b><time>{occurred.date} · {occurred.time}</time></header><p>Виготовлено: {product.quantity} {product.measurementUnit}</p><small>Списано: {product.ingredients.map((part) => `${part.equipmentName} — ${part.quantity} ${part.measurementUnit}`).join("; ")}</small>{product.notes && <small>Примітка: {product.notes}</small>}</div></article>; }) : <div className="personnel-state"><History /><b>Історія порожня</b><span>Виготовлення з’являться тут після першої облікової операції.</span></div>}</div></Modal>}
  </PageFrame>;
}
