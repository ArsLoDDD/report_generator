import { ChevronRight, ClipboardList, FileText, Network, Route, Settings2, UsersRound } from "lucide-react";
...
function PersonCard({ person }: { person: StaffingRecord }) {
...
function VacancyCard({ position, recommendation, actingPerson, onRecommendation }: { position: string; recommendation?: VacancyRecommendation; actingPerson?: StaffingRecord; onRecommendation: () => void }) {
  return <article className="staff-vacancy-card"><div><span>Вільна посада</span><b>{position}</b>{recommendation && <small>Рек. лист: {recommendation.fullName} · {recommendation.issuedAt}</small>}</div><div className="staff-vacancy-card__status"><span>Тимчасове виконання</span><b className={actingPerson ? "staff-vacancy-card__acting" : ""}>{actingPerson ? actingPerson.fullName : "ТВО відсутній"}</b>{actingPerson && <small>{actingPerson.rank || "Без звання"}</small>}</div><button className="button" onClick={onRecommendation}><FileText />Рек. лист</button></article>;
}

function StaffGroupRows({ group, records, slots, recommendations, onRecommendation }: { group: ReturnType<typeof buildStaffingHierarchy>[number]["groups"][number]; records: StaffingRecord[]; slots: StaffSlot[]; recommendations: VacancyRecommendation[]; onRecommendation: (slot: StaffSlot) => void }) {
  return <div className="staff-people">{group.items.map((item) => item.kind === "person" ? <PersonCard key={item.person.personnelId} person={item.person} /> : <VacancyCard key={item.slot.id} position={item.position} actingPerson={actingForSlot(item.slot, records, slots)[0]} recommendation={recommendations.find((recommendation) => recommendation.slotId === item.slot.id)} onRecommendation={() => onRecommendation(item.slot)} />)}</div>;
}
...
export function StaffingBcsPage() {
  ...
  const [bcsDate, setBcsDate] = useState(() => { const date = new Date(); date.setDate(date.getDate() + 1); return `${String(date.getDate()).padStart(2, "0")}.${String(date.getMonth() + 1).padStart(2, "0")}.${date.getFullYear()}`; });
  const [bcsUnitName, setBcsUnitName] = useState("");
  const [tab, setTab] = useState<"staff" | "bcs">("staff");
  const [transferOpen, setTransferOpen] = useState(false);
  const [vacancyFor, setVacancyFor] = useState<StaffSlot | null>(null);
  const reload = useCallback(async () => { const [nextRecords, nextSettings, recommendations, externals, nextCrews] = await Promise.all([operationsService.listStaffingRecords(), settingsService.get(), operationsService.listVacancyRecommendations(), operationsService.listTemporaryPersonnel(), operationsService.listCrews()]); const nextUnit=nextSettings.unit ?? { kind: "Рота" as const, shortName: "", authorizedStrength: 0 }; setRecords(nextRecords); setUnit(nextUnit); setBcsUnitName((current)=>current||nextUnit.shortName||nextUnit.fullName||""); setVacancyRecommendations(recommendations); setTemporaryPeople(externals); setCrews(nextCrews); }, []);
  useEffect(() => { void reload(); }, [reload]);
  const structure = useMemo(() => structureWithUnmappedPositions(unit, records.map((person) => person.position)), [unit, records]);
  ...
  return <PageFrame className={`staffing-page ${tab === "bcs" ? "staffing-page--bcs" : ""}`} header={tab === "staff" ? <PageTitle title="Штат та БЧС" subtitle="Штат формується з посад, а БЧС — лише з актуальних даних програми" actions={<div className="staffing-actions"><button className="button primary" onClick={() => setTransferOpen(true)} disabled={!records.length}><Route />Переміщення</button></div>} /> : undefined} tools={<div className="staffing-tools"><div className="staffing-tabs"><button className={tab === "staff" ? "active" : ""} onClick={() => setTab("staff")}><UsersRound />Штат</button><button className={tab === "bcs" ? "active" : ""} onClick={() => setTab("bcs")}><ClipboardList />БЧС</button></div>{tab === "bcs" && <button className="button bcs-parameters-button" onClick={() => setParametersOpen(true)}><Settings2 />Параметри БЧС</button>}</div>}>
    {tab === "staff" ? <div className="staff-tree"><section className="panel staff-overview"><Network /><div><b>{unit.shortName || "Підрозділ"}</b><span>{unit.kind} · за штатом {unit.authorizedStrength || "не вказано"} · за списком {records.length}</span></div><strong>{records.length}/{unit.authorizedStrength || "—"}</strong></section><section className="staff-structure" aria-label="Структура штатних посад">{hierarchy.map((section) => <section className="staff-company" key={section.section}><header><div><h2>{section.section}</h2><span>{section.groups.reduce((sum, group) => sum + group.people.length, 0)} осіб · {section.groups.reduce((sum, group) => sum + group.vacancies.length, 0)} вільних</span></div></header><div className="staff-platoons">{section.groups.map((group) => <article key={group.name}><h3><ChevronRight />{group.name}</h3><StaffGroupRows group={group} records={records} slots={slots} recommendations={vacancyRecommendations} onRecommendation={setVacancyFor} /></article>)}</div></section>)}</section><section className="panel recommendation-history"><header><FileText /><div><h2>Рекомендаційні листи</h2><p>Видано кандидатам на вільні штатні посади.</p></div></header>{vacancyRecommendations.length ? <div>{vacancyRecommendations.map((item) => <article key={`vacancy-${item.id}`}><b>{item.fullName}</b><span>Вільна посада · {item.positionName}</span><time>{item.issuedAt}</time><small>{[item.rank, item.phone].filter(Boolean).join(" · ") || "Дані не вказані"}</small></article>)}</div> : <p>Рекомендаційних листів ще немає.</p>}</section></div> : <div className="bcs-board"><BcsTable records={bcsRecords} authorized={unit.authorizedStrength} zoom={bcsZoom} onSave={saveBcsPerson} /></div>}
    {externalDraft && <TemporaryArrivalEditor initial={externalDraft} onClose={() => setExternalDraft(null)} onSave={async (person) => { try { await operationsService.saveTemporaryPersonnel(person); setExternalDraft(null); await reload(); notify("Запис БЧС збережено.", "success"); } catch (error) { notify(String(error), "error"); } }} />}
    {parametersOpen && <BcsParametersModal unitName={bcsUnitName} date={bcsDate} zoom={bcsZoom} people={temporaryPeople} onUnitName={setBcsUnitName} onDate={setBcsDate} onZoom={setZoom} onAdd={(category) => { setParametersOpen(false); setExternalDraft({ category }); }} onEdit={(person) => { setParametersOpen(false); setExternalDraft(person); }} onDelete={deleteExternal} onExport={() => void exportBcs()} onClose={() => setParametersOpen(false)} />}
