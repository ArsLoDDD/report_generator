...
export function StaffTransferModal({ records, slots, onClose, onSave }: { records: StaffingRecord[]; slots: StaffSlot[]; onClose: () => void; onSave: (moves: SlotTransfer[], acting: ActingChange[]) => Promise<void> }) {
  ...
  return <Modal title="Зміни у штаті" subtitle="Переміщення між посадами та призначення тимчасового виконання обов’язків" onClose={onClose} className="staff-transfer-modal">
    ...
    {actingDecision && <section className="transfer-current" role="alert"><b>На цій посаді є ТВО: {actingDecision.incumbents.map((person) => person.fullName).join(", ")}</b><p>Зняти ТВО після призначення постійного військовослужбовця?</p><button data-modal-submit className="button primary" onClick={() => { setActing((current) => [...current.filter((item) => !actingDecision.incumbents.some((person) => person.personnelId === item.personnelId)), ...actingDecision.incumbents.map((person) => ({ personnelId: person.personnelId, slotId: "", position: "" }))]); stageMove(actingDecision.slot, actingDecision.person); setActingDecision(null); }}>Зняти ТВО та призначити</button><button className="button" onClick={() => setActingDecision(null)}>Скасувати призначення</button></section>}
