...
export function FlightJournalPage() {
  ...
  const save = async () => {
    ...
    try { await operationsService.createFlightJournalEntry(persistedDraft); planRequestRef.current += 1; setOpen(false); setDraft(emptyDraft()); await reload(); notify("Запис журналу польотів збережено.", "success"); } catch (error) { notify(typeof error === "string" ? error : "Не вдалося зберегти запис.", "error"); }
  };
  ...
  return <PageFrame className="flight-journal-page" header={<PageTitle title="Журнал польотів" subtitle="Фактичні польоти екіпажів із даними БпЛА та БК" actions={<button className="button primary" onClick={() => { planRequestRef.current += 1; setDraft(emptyDraft()); setOpen(true); }}><Plus />Додати</button>} />}>
    ...
    {open && <Modal title="Новий запис польоту" subtitle="Поля з плану та картки екіпажу можна змінити перед збереженням." onClose={() => { planRequestRef.current += 1; setOpen(false); }} className="flight-journal-editor"><div className="operation-editor__body">
    ...
    </div><footer className="modal-actions"><button className="button" onClick={() => { planRequestRef.current += 1; setOpen(false); }}>Скасувати</button><button className="button primary" onClick={() => void save()}>Зберегти запис</button></footer></Modal>}
