...
import type { Crew, Equipment, FlightPlanEntry, WorkshopProduct } from "./types";
...
export function FlightPlanningPage(){
  ...
  const rows=useMemo(()=>flightPlanPreviewRows(unitName,selected,entries,crews,uavs,vehicles,ammunition,workshopProducts),[ammunition,crews,entries,selected,uavs,unitName,vehicles,workshopProducts]);
  ...
  return <PageFrame className="flight-planning-page" header={<PageTitle title="План польотів" subtitle="Підготовка екіпажів, маршрутів і засобів для виконання завдань" />} tools={<div className="flight-plan-toolbar"><span>Вибрано екіпажів: <b>{selected.length}</b></span><button className="button" onClick={()=>setParametersOpen(true)}><Settings2/>Параметри плану польотів</button></div>}>
    {!loaded?<CardGridSkeleton count={4}/>:<div className="flight-planning-layout"><div className="flight-planning-layout__preview"><FlightPlanTable rows={rows} zoom={zoom}/></div><div className="flight-planning-layout__settings"><FlightPlanParametersModal crews={crews} vehicles={vehicles} uavs={uavs} ammunition={ammunition} workshopProducts={workshopProducts} selected={selected} entries={entries} missingCallsignCount={missingCallsigns.length} onToggle={toggleCrew} onPatch={patchEntry}/></div></div>}
