...
import { flightPlanPreviewRows, initialFlightEntry, initialWeather, isFlightPlanMemberAvailable, missingCrewCallsigns, reverseFlightPlanPersonnelTransition, reverseFlightPlanPersonnelTransitions, validateFlightPlanPersonnelTransitions, validateFlightPlanSchedule } from "./flight-plan-model";
import { FLIGHT_PLAN_PENDING_STORAGE_KEY, FLIGHT_PLAN_STORAGE_KEY } from "./flight-plan-storage";
import { operationsService } from "./services/operationsService";
import type { Crew, Equipment, FlightPlanEntry, FlightPlanPersonnelTransition, FlightPlanRequest, FlightPlanRotation, Position, WorkshopProduct } from "./types";
...
type StoredDraft={schemaVersion?:number;unitName?:string;date?:string;zoom?:number;selected?:number[];entries?:Record<number,FlightPlanEntry>;rotations?:Record<number,FlightPlanRotation[]>;personnelTransitions?:FlightPlanPersonnelTransition[];rolledFromPreviousDate?:boolean;pendingSave?:PendingSaveMetadata};
...
const normalisePersonnelTransition=(value:unknown):FlightPlanPersonnelTransition|null=>{
  if(!isRecord(value)||typeof value.id!=="string"||typeof value.crewId!=="number"||!Number.isFinite(value.crewId))return null;
  const memberSnapshots=Array.isArray(value.memberSnapshots)?value.memberSnapshots.flatMap((member)=>isRecord(member)&&typeof member.personnelId==="number"&&typeof member.fullName==="string"&&typeof member.rank==="string"?[{personnelId:member.personnelId,fullName:member.fullName,rank:member.rank}]:[]):[];
  return{id:value.id,crewId:value.crewId,outgoingMemberIds:numberArray(value.outgoingMemberIds),outgoingTime:typeof value.outgoingTime==="string"?value.outgoingTime:"",incomingMemberIds:numberArray(value.incomingMemberIds),incomingTime:typeof value.incomingTime==="string"?value.incomingTime:"",memberSnapshots};
};
...
const parseSnapshot=(value:string|null|undefined):FlightPlanRequest|null=>{if(value==null||value.trim()==="")return null;const parsed:unknown=JSON.parse(value);if(!isRecord(parsed)||!Array.isArray(parsed.entries))throw new Error("Invalid flight plan snapshot");return{unitName:typeof parsed.unitName==="string"?parsed.unitName:"Підрозділ",entries:parsed.entries.map((entry)=>normaliseEntry(entry)),personnelTransitions:Array.isArray(parsed.personnelTransitions)?parsed.personnelTransitions.flatMap((item)=>{const transition=normalisePersonnelTransition(item);return transition?[transition]:[]}):[]};};
...
const transitionEligibleMemberIds=(crew:Crew,transitions:FlightPlanPersonnelTransition[])=>[...new Set([
  ...crew.actualMembers.map((member)=>member.personnelId),
  ...transitions.flatMap((transition)=>transition.memberSnapshots?.map((member)=>member.personnelId)??[]),
])];
...
export function FlightPlanningPage(){
  ...
  const [personnelTransitions,setPersonnelTransitions]=useState<FlightPlanPersonnelTransition[]>(initial.personnelTransitions??[]);
  ...
  const load=useCallback(async()=>{
    ...
    const unavailablePositionIds=new Set(nextPositions.filter((position)=>position.positionType==="Облаштовується").map((position)=>position.id));
    const requestedIsHistorical=dateNumber(requestedDate)<dateNumber(todayAtLoad);
    const available=requested.filter((id)=>nextCrews.some((crew)=>crew.id===id&&(
      requestedIsHistorical||crew.positionId==null||!unavailablePositionIds.has(crew.positionId)
    )));
    ...
    const nextPersonnelTransitions=(localDraft?.personnelTransitions??snapshot?.personnelTransitions??[]).filter((transition)=>nextCrews.some((crew)=>crew.id===transition.crewId));
    setEntries(nextEntries);setRotations(nextRotations);setPersonnelTransitions(nextPersonnelTransitions);setSelected(nextSelected);
  ...
  const personnelTransitionValidations=useMemo(()=>Object.fromEntries(selected.flatMap((crewId)=>{const crew=crews.find((item)=>item.id===crewId);const crewRotations=rotations[crewId]??[];const finalStage=crewRotations[crewRotations.length-1]??entries[crewId];if(!crew||!finalStage)return[];const crewTransitions=personnelTransitions.filter((transition)=>transition.crewId===crewId);const initialIds=reverseFlightPlanPersonnelTransitions(finalStage.actualMemberIds,crewTransitions,crewId);const primary=entries[crewId];const stageStartTime=crewRotations.length?finalStage.startTime:primary?.arrivesToday?primary.startTime:"00:00";return[[crewId,validateFlightPlanPersonnelTransitions(initialIds,crewTransitions,crewId,transitionEligibleMemberIds(crew,crewTransitions),{stageStartTime,departureTime:primary?.departsToday?primary.departureTime:undefined})]];})),[crews,entries,personnelTransitions,rotations,selected]);
  const hasInvalidPersonnelTransitions=selected.some((crewId)=>personnelTransitionValidations[crewId]&&!personnelTransitionValidations[crewId].isValid);
  ...
  const unavailablePositionCrewIds=useMemo(()=>{if(isHistoricalPlan)return[];const blockedPositionIds=new Set(positions.filter((position)=>position.positionType==="Облаштовується").map((position)=>position.id));return crews.filter((crew)=>crew.positionId!=null&&blockedPositionIds.has(crew.positionId)).map((crew)=>crew.id);},[crews,isHistoricalPlan,positions]);
  const invalidSelectedPositionState=selected.some((crewId)=>unavailablePositionCrewIds.includes(crewId));
  const selectedEntries=useMemo(()=>selected.flatMap((id)=>{const crew=crews.find((item)=>item.id===id);const entry=entries[id];if(!crew||!entry)return[];const position=positions.find((item)=>item.id===crew.positionId);const withoutVehicle=Boolean(entry.withoutVehicle||(rotations[id]??[]).some((stage)=>stage.withoutVehicle));const snapshot=(stage:FlightPlanEntry):FlightPlanEntry=>({...stage,withoutVehicle,actualVehicleId:withoutVehicle?null:stage.actualVehicleId,crewName:isHistoricalPlan&&hasOwn(stage,"crewName")?stage.crewName:crew.name,crewUavType:isHistoricalPlan&&hasOwn(stage,"crewUavType")?stage.crewUavType:crew.uavType,positionId:isHistoricalPlan&&hasOwn(stage,"positionId")?stage.positionId:position?.id??crew.positionId,positionName:isHistoricalPlan&&hasOwn(stage,"positionName")?stage.positionName:position?.name||crew.positionName,positionMgrs:isHistoricalPlan&&hasOwn(stage,"positionMgrs")?stage.positionMgrs:position?.mgrs||"",positionLocality:isHistoricalPlan&&hasOwn(stage,"positionLocality")?stage.positionLocality:position?.locality||"",workStrip:isHistoricalPlan&&hasOwn(stage,"workStrip")?stage.workStrip:position?.stripName||crew.sector,battleOrder:isHistoricalPlan&&hasOwn(stage,"battleOrder")?stage.battleOrder:position?.battleOrder||crew.battleOrder,uavSnapshots:isHistoricalPlan&&hasOwn(stage,"uavSnapshots")?stage.uavSnapshots:(stage.uavSelections??[]).flatMap((selection)=>{const uav=uavs.find((item)=>item.id===selection.equipmentId);return uav?[{equipmentId:uav.id,name:uav.name,serialNumber:uav.inventoryNumber}]:stage.uavSnapshots?.filter((item)=>item.equipmentId===selection.equipmentId)??[];}),memberSnapshots:isHistoricalPlan&&hasOwn(stage,"memberSnapshots")?stage.memberSnapshots:(stage.actualMemberIds??[]).flatMap((personnelId)=>{const previous=stage.memberSnapshots?.find((item)=>item.personnelId===personnelId);const member=[...(crew.members??[]),...(crew.actualMembers??[])].find((item)=>item.personnelId===personnelId);if(member)return[{personnelId,fullName:member.fullName,rank:member.rank}];return previous?[previous]:[];})});return[snapshot(entry),...(rotations[id]??[]).map(snapshot)];}),[crews,entries,isHistoricalPlan,positions,rotations,selected,uavs]);
  const selectedPersonnelTransitions=useMemo(()=>personnelTransitions.filter((transition)=>selected.includes(transition.crewId)),[personnelTransitions,selected]);
  const saveValid=!hasInvalidSelectedSchedule&&!hasInvalidPersonnelTransitions&&!invalidSelectedMemberState.length&&!invalidSelectedPositionState&&/^\d{2}\.\d{2}\.\d{4}$/u.test(date);
  const activeDraft:StoredDraft={schemaVersion:3,unitName,date,zoom,selected,entries,rotations,personnelTransitions,rolledFromPreviousDate:(rolledSeedDate===date&&!dirtyRef.current)||undefined,pendingSave:dirtyRef.current?{date:isoDate(date),revision:editRevisionRef.current,updatedAt:pendingUpdatedAtRef.current}:undefined};
  ...
  useEffect(()=>{if(!loaded||loadedDate!==date||loadFailed||!dirtyRef.current||!saveValid)return;const timer=window.setTimeout(()=>{void drainSaveQueue();},350);return()=>window.clearTimeout(timer);},[date,drainSaveQueue,entries,loaded,loadedDate,loadFailed,personnelTransitions,rotations,saveValid,selected,unitName]);
  ...
  const patchCrewVehicleMode=(crewId:number,withoutVehicle:boolean)=>{
    setEntries((current)=>({...current,[crewId]:{...current[crewId],withoutVehicle,actualVehicleId:withoutVehicle?null:current[crewId]?.actualVehicleId??null}}));
    setRotations((current)=>({...current,[crewId]:(current[crewId]??[]).map((rotation)=>({...rotation,withoutVehicle,actualVehicleId:withoutVehicle?null:rotation.actualVehicleId}))}));
  };
  const patchEntry=(crewId:number,patch:Partial<FlightPlanEntry>)=>{markDirty();if(hasOwn(patch,"withoutVehicle")){patchCrewVehicleMode(crewId,patch.withoutVehicle===true);return;}if(hasOwn(patch,"actualMemberIds"))setPersonnelTransitions((current)=>current.filter((transition)=>transition.crewId!==crewId));setEntries((current)=>({...current,[crewId]:{...current[crewId],...patch}}));};
  const patchRotation=(crewId:number,rotationId:string,patch:Partial<FlightPlanEntry>)=>{markDirty();if(hasOwn(patch,"withoutVehicle")){patchCrewVehicleMode(crewId,patch.withoutVehicle===true);return;}if(hasOwn(patch,"actualMemberIds"))setPersonnelTransitions((current)=>current.filter((transition)=>transition.crewId!==crewId));setRotations((current)=>({...current,[crewId]:(current[crewId]??[]).map((rotation)=>rotation.rotationId===rotationId?{...rotation,...patch}:rotation)}));};
  const saveRotation=(crewId:number,memberIds:number[],commanderId:number,rotationId?:string)=>{if(personnelTransitions.some((transition)=>transition.crewId===crewId)){notify("Спочатку видаліть точкові зміни ОС цього екіпажу, а потім створюйте або редагуйте повну ротацію.","error");return;}markDirty();setRotations((current)=>{const existing=current[crewId]??[];if(rotationId)return{...current,[crewId]:existing.map((rotation)=>rotation.rotationId===rotationId?{...rotation,actualMemberIds:memberIds,actualCommanderId:commanderId}:rotation)};const source=existing[existing.length-1]??entries[crewId];if(!source)return current;const next={...cloneEntry(source),rotationId:`${crewId}-${Date.now()}`,actualMemberIds:memberIds,actualCommanderId:commanderId,startTime:nextRotationTime(source),arrivesToday:false,departsToday:false,departureTime:""};return{...current,[crewId]:[...existing,next]};});};
  const savePersonnelTransition=(transition:FlightPlanPersonnelTransition)=>{
    const {crewId,outgoingMemberIds,outgoingTime,incomingMemberIds,incomingTime}=transition;
    const crew=crews.find((item)=>item.id===crewId);if(!crew)return;
    const stages=rotations[crewId]??[];const currentStage=stages[stages.length-1]??entries[crewId];if(!currentStage)return;
    const outgoing=new Set(outgoingMemberIds);const nextMemberIds=[...new Set([...currentStage.actualMemberIds.filter((id)=>!outgoing.has(id)),...incomingMemberIds])];
    const allMembers=uniqueCrewMembers(crew);const memberSnapshots=transition.memberSnapshots?.length?transition.memberSnapshots:[...new Set([...outgoingMemberIds,...incomingMemberIds])].flatMap((personnelId)=>{const member=allMembers.find((item)=>item.personnelId===personnelId);return member?[{personnelId,fullName:member.fullName,rank:member.rank}]:[];});
    const currentCommander=currentStage.actualCommanderId;const nextCommander=currentCommander!=null&&nextMemberIds.includes(currentCommander)?currentCommander:allMembers.find((member)=>nextMemberIds.includes(member.personnelId)&&member.position.toLocaleLowerCase("uk").includes("командир"))?.personnelId??nextMemberIds[0]??null;
    const savedTransition:FlightPlanPersonnelTransition={...transition,outgoingMemberIds:[...outgoingMemberIds],outgoingTime:outgoingMemberIds.length?outgoingTime:"",incomingMemberIds:[...incomingMemberIds],incomingTime:incomingMemberIds.length?incomingTime:"",memberSnapshots};
    const crewTransitions=[...personnelTransitions.filter((item)=>item.crewId===crewId),savedTransition];const initialIds=reverseFlightPlanPersonnelTransitions(nextMemberIds,crewTransitions,crewId);const primary=entries[crewId];const stageStartTime=stages.length?currentStage.startTime:primary?.arrivesToday?primary.startTime:"00:00";const validation=validateFlightPlanPersonnelTransitions(initialIds,crewTransitions,crewId,transitionEligibleMemberIds(crew,crewTransitions),{stageStartTime,departureTime:primary?.departsToday?primary.departureTime:undefined});if(!validation.isValid){notify(validation.errors[0],"error");return;}
    markDirty();setPersonnelTransitions((current)=>[...current,savedTransition]);
    if(stages.length)setRotations((current)=>({...current,[crewId]:(current[crewId]??[]).map((stage,index,list)=>index===list.length-1?{...stage,actualMemberIds:nextMemberIds,actualCommanderId:nextCommander}:stage)}));
    else setEntries((current)=>({...current,[crewId]:{...current[crewId],actualMemberIds:nextMemberIds,actualCommanderId:nextCommander}}));
  };
  const deletePersonnelTransition=(crewId:number,transitionId:string)=>{const crewTransitions=personnelTransitions.filter((transition)=>transition.crewId===crewId);const transition=crewTransitions[crewTransitions.length-1];if(!transition||transition.id!==transitionId){notify("Зміни ОС видаляються від останньої до першої, щоб не порушити хронологію.","error");return;}const crew=crews.find((item)=>item.id===crewId);const stages=rotations[crewId]??[];const currentStage=stages[stages.length-1]??entries[crewId];if(!crew||!currentStage)return;const previousMemberIds=reverseFlightPlanPersonnelTransition(currentStage.actualMemberIds,transition);const allMembers=uniqueCrewMembers(crew);const commander=currentStage.actualCommanderId!=null&&previousMemberIds.includes(currentStage.actualCommanderId)?currentStage.actualCommanderId:allMembers.find((member)=>previousMemberIds.includes(member.personnelId)&&member.position.toLocaleLowerCase("uk").includes("командир"))?.personnelId??previousMemberIds[0]??null;markDirty();setPersonnelTransitions((current)=>current.filter((item)=>item.id!==transitionId));if(stages.length)setRotations((current)=>({...current,[crewId]:(current[crewId]??[]).map((stage,index,list)=>index===list.length-1?{...stage,actualMemberIds:previousMemberIds,actualCommanderId:commander}:stage)}));else setEntries((current)=>({...current,[crewId]:{...current[crewId],actualMemberIds:previousMemberIds,actualCommanderId:commander}}));};
  const deleteRotation=(crewId:number,rotationId:string)=>{if(personnelTransitions.some((transition)=>transition.crewId===crewId)){notify("Спочатку видаліть точкові зміни ОС цього екіпажу.","error");return;}const existing=rotations[crewId]??[];const index=existing.findIndex((rotation)=>rotation.rotationId===rotationId);const previous=index>0?existing[index-1]:entries[crewId];const next=existing[index+1];if(previous&&next&&sameComposition(previous,next)){notify(`Не можна видалити ротацію ${index+1} екіпажу «${crews.find((crew)=>crew.id===crewId)?.name||crewId}»: сусідні етапи матимуть однаковий склад.`,"error");return;}markDirty();setRotations((current)=>({...current,[crewId]:(current[crewId]??[]).filter((rotation)=>rotation.rotationId!==rotationId)}));};
  const toggleCrew=(crewId:number)=>{
    ...
    if(!isHistoricalPlan&&!selected.includes(crewId)&&unavailablePositionCrewIds.includes(crewId)){
      const crewName=crews.find((crew)=>crew.id===crewId)?.name||String(crewId);
      notify(`Екіпаж «${crewName}» закріплений за позицією, яка ще облаштовується. Завершіть облаштування або оберіть екіпажу іншу позицію.`,"error");
      return;
    }
  ...
  const exportPlan=async()=>{
    ...
    if(invalidSelectedPositionState){const crew=crews.find((item)=>selected.includes(item.id)&&unavailablePositionCrewIds.includes(item.id));notify(`Екіпаж «${crew?.name||"обраний екіпаж"}» закріплений за позицією, яка ще облаштовується.`,"error");return;}
    ...
    for(const crewId of selected){
      ...
      const transitionValidation=personnelTransitionValidations[crewId];
      if(transitionValidation&&!transitionValidation.isValid){notify(`Екіпаж «${crewName}»: ${transitionValidation.errors[0]}`,"error");return;}
    ...
    if(!path)return;
    setExporting(true);
    try{await operationsService.exportFlightPlan(path.endsWith(".xlsx")?path:`${path}.xlsx`,{unitName,entries:selectedEntries,personnelTransitions:selectedPersonnelTransitions});notify("План польотів сформовано за наданим шаблоном.","success");}catch(error){notify(typeof error==="string"?error:"Не вдалося сформувати план польотів.","error");}finally{setExporting(false);}
  };
  ...
  return <PageFrame className="flight-planning-page" header={<PageTitle title="План польотів" subtitle="Підготовка екіпажів, маршрутів і засобів для виконання завдань" />} tools={<div className="flight-plan-toolbar"><div className="flight-plan-date-navigator" aria-label="Перемикання планів за датою"><button type="button" className="icon-button" aria-label="Попередній день" disabled={!loaded||isoDate(date)<=dateBounds.min} onClick={()=>shiftSelectedDate(-1)}><ChevronLeft/></button><label className="flight-plan-date-navigator__field"><CalendarDays/><span>Дата плану</span><input aria-label="Дата плану" type="date" min={dateBounds.min} max={dateBounds.max} value={isoDate(date)} disabled={!loaded} onChange={(event)=>void selectPlanDate(event.target.value)}/></label><button type="button" className="icon-button" aria-label="Наступний день" disabled={!loaded||isoDate(date)>=dateBounds.max} onClick={()=>shiftSelectedDate(1)}><ChevronRight/></button><button type="button" className="button compact" disabled={!loaded||isoDate(date)===today} onClick={()=>void selectPlanDate(today)}>Сьогодні</button><button type="button" className="button compact" disabled={!loaded||isoDate(date)===shiftIsoDate(today,1)} onClick={()=>void selectPlanDate(shiftIsoDate(today,1))}>Завтра</button></div><div className="flight-plan-toolbar__actions"><span className={`flight-plan-save-status is-${saveStatus}`} role="status">{statusText}</span>{saveStatus==="save-error"&&<button type="button" className="button compact" onClick={()=>void drainSaveQueue()}>Повторити збереження</button>}<span>Вибрано екіпажів: <b>{selected.length}</b></span><button className="button" disabled={loadFailed} onClick={()=>setParametersOpen(true)}><Settings2/>Параметри плану польотів</button></div></div>}>
    {!loaded?<CardGridSkeleton count={4}/>:loadFailed?<section className="panel flight-plan-load-error" role="alert"><h2>План за {date} не завантажено</h2><p>Локальні незбережені зміни не видалено. Повторіть завантаження або оберіть іншу дату.</p><button type="button" className="button" onClick={()=>setReloadKey((value)=>value+1)}>Повторити</button></section>:<div className="flight-planning-layout"><div className="flight-planning-layout__preview"><FlightPlanTable rows={rows} zoom={zoom}/></div><div className="flight-planning-layout__settings"><FlightPlanParametersModal crews={crews} vehicles={vehicles} uavs={uavs} ammunition={ammunition} workshopProducts={workshopProducts} selected={selected} entries={entries} rotations={rotations} personnelTransitions={personnelTransitions} personnelTransitionErrors={Object.fromEntries(Object.entries(personnelTransitionValidations).map(([crewId,validation])=>[Number(crewId),validation.errors]))} validations={scheduleValidations} missingCallsignCount={missingCallsigns.length} unavailablePositionCrewIds={unavailablePositionCrewIds} onToggle={toggleCrew} onPatch={patchEntry} onPatchRotation={patchRotation} onSaveRotation={saveRotation} onDeleteRotation={deleteRotation} onSavePersonnelTransition={savePersonnelTransition} onDeletePersonnelTransition={deletePersonnelTransition}/></div></div>}
