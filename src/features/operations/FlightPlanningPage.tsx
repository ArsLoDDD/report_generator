...
export function FlightPlanningPage(){
  ...
  const patchEntry=(crewId:number,patch:Partial<FlightPlanEntry>)=>{markDirty();if(hasOwn(patch,"withoutVehicle")){patchCrewVehicleMode(crewId,patch.withoutVehicle===true);return;}setEntries((current)=>({...current,[crewId]:{...current[crewId],...patch}}));};
  const patchRotation=(crewId:number,rotationId:string,patch:Partial<FlightPlanEntry>)=>{markDirty();if(hasOwn(patch,"withoutVehicle")){patchCrewVehicleMode(crewId,patch.withoutVehicle===true);return;}setRotations((current)=>({...current,[crewId]:(current[crewId]??[]).map((rotation)=>rotation.rotationId===rotationId?{...rotation,...patch}:rotation)}));};
