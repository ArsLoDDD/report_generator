...
import { Check, FileText, FolderOpen, LoaderCircle, Replace, WandSparkles } from "lucide-react";
...
type ManualReplacement = { id: string; value: string; replacement: string };
...
export function ReportAnalyserPage({ onCreated, onOpenConstructor }: { onCreated: (templatePath: string) => void; onOpenConstructor: () => void }) {
  ...
  const replacements = useMemo<TemplateAnalysisReplacement[]>(() => [
    ...selectedProposals.map((proposal) => ({ value: proposal.value, token: proposal.token })),
    ...manual.map(({ value, replacement }) => ({ value, token: "", replacement }))
  ], [manual, selectedProposals]);
  useEffect(() => {
  ...
  const analyse = async (reportPath: string) => {
    ...
    try {
      ...
      setAnalysis(result); setSelected(result.proposals.map(proposalKey)); setManual([]); setSelectedText("");
  ...
  const applyManualReplacement = () => {
    ...
    if (!value.trim() || !replacement.trim()) { notify("Виділіть текст у документі та вкажіть, на що його замінити.", "info"); return; }
    setManual((current) => [...current.filter((item) => item.value !== value), { id: crypto.randomUUID(), value, replacement }]);
    setSelectedText(""); setReplacementInput("");
  };
