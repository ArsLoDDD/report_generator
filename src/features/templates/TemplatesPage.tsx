...
import { getGenerationParameter } from "../../shared/template-language/registry";
...
export function TemplatesPage({ templates, totalCount, hasMore, isRefreshing: isBackgroundRefreshing, isLoadingMore, onLoadMore, selected, onSelect, onRefresh }: TemplatesPageProps) {
  ...
  const variableGroups = useMemo(() => ({
    document: inspection.variables.filter((value) => Boolean(getGenerationParameter(value.split(":")[0]))),
    people: inspection.variables.filter((value) => value.startsWith("військовий_")),
    signers: inspection.variables.filter((value) => !getGenerationParameter(value.split(":")[0]) && !value.startsWith("військовий_"))
  ...
  return <PageFrame className="templates-page"><div className="templates-layout">
    ...
    <section className="panel template-details"><div className="template-details__scroll">
      ...
      <div className="detail-cards"><article className="variables-card"><header className="variables-card__title"><div><h3>Використовувані змінні</h3><p>Прочитані безпосередньо з обраного DOCX-файлу.</p></div><span>{inspection.variables.length}</span></header>{variableGroups.document.length > 0 && <VariableGroup icon={FileText} label="Параметри документа" hint="Значення перед генерацією" values={variableGroups.document.map((value) => `{{${value}}}`)} />}{variableGroups.people.length > 0 && <VariableGroup icon={Users} label="Військовослужбовці" hint="Обрані особи" tone="collection" values={variableGroups.people.map((value) => `{{${value}}}`)} />}{variableGroups.signers.length > 0 && <VariableGroup icon={FolderOpen} label="Підписанти" hint="Дані з налаштувань" values={variableGroups.signers.map((value) => `{{${value}}}`)} />}{inspection.variables.length === 0 && <p className="variables-empty">У шаблоні не знайдено змінних у форматі {"{{...}}"}.</p>}</article><article className="template-side"><h3>Деталі шаблону</h3><dl><dt>Тип файлу:</dt><dd>DOCX</dd><dt>Стан:</dt><dd>{inspection.isValid ? "Готовий" : "Потрібна перевірка"}</dd><dt>Параметри документа:</dt><dd>{variableGroups.document.length ? "Користувач заповнює значення" : "Не потрібні"}</dd></dl><div className="recent-template-reports"><header><div><h3>Останні рапорти</h3><p>Створені за цим шаблоном</p></div></header><RecentReportsList reports={recentReports} onOpen={(reportPath) => void openRecentReport(reportPath)} /></div></article></div>
    </div></section>
  </div>{templateToDelete && <ConfirmDialog title="Видалити шаблон?" message={`DOCX-файл «${templateToDelete.name}» буде видалено з папки «Шаблони». Цю дію не можна скасувати.`} confirmLabel="Видалити" onConfirm={() => void confirmDeleteTemplate()} onCancel={() => setTemplateToDelete(null)} busy={isDeleting} />}</PageFrame>;
}
