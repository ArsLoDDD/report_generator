...
/** One reusable form for every person whose data may be inserted into a template. */
export function SignerForm({ number, role, title, value, isSaving, onSave }: Props) {
  ...
  return <article className="signer-card"><header><b>{number}. {title}</b></header><div className="signer-fields"><label>ПІБ<input value={draft.fullName} onChange={(event) => change("fullName", event.target.value)} /></label><label>Звання<input value={draft.rank} onChange={(event) => change("rank", event.target.value)} /></label><label>Посада<input value={draft.position} onChange={(event) => change("position", event.target.value)} /></label></div><footer><button className="button primary" onClick={() => void submit()} disabled={isSaving}><Save />{isSaving ? "Збереження…" : "Зберегти"}</button></footer></article>;
}
