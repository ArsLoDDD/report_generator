...
export function SettingsPage() {
  ...
  return <PageFrame header={<PageTitle title="Налаштування" subtitle="Підписанти, Excel-база та перенесення даних" />} className="settings-page">
    ...
    {excelOpen && <Modal title="Імпорт Excel-бази даних" onClose={() => setExcelOpen(false)}><p>Оберіть спосіб імпорту записів.</p><button className="personnel-import-option" onClick={() => void importExcel("append")}><b>Доповнити БД</b><span>Додати записи до наявних.</span></button><button className="personnel-import-option personnel-import-option--danger" onClick={() => void importExcel("replace")}><b>Замінити БД</b><span>Повністю замінити записи.</span></button></Modal>}
