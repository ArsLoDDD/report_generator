...
export function SettingsPage() {
  ...
  const importExcel = async (mode: "append" | "replace") => { try { setExcelImportError(""); setExcelImportResult(null); const path = await open({ title: "Імпорт Excel-бази даних", filters: [{ name: "Таблиця Excel", extensions: ["xlsx"] }] }); if (!path || Array.isArray(path)) return; const result = await personnelService.importExcel(path, mode); if (result.warnings.length) { setExcelImportResult(result); notify("Excel-базу імпортовано із зауваженнями.", "warning"); return; } setExcelOpen(false); notify("Excel-базу імпортовано.", "success"); window.location.reload(); } catch (error) { const message = error instanceof Error ? error.message : typeof error === "string" ? error : "Не вдалося імпортувати Excel-базу даних."; console.error("Excel import failed", { mode, error }); setExcelImportError(message); notify(message, "error"); } };
  ...
  const importArchive = async () => { try { const path = await open({ title: "Імпорт архіву даних", filters: [{ name: "Архів перенесення", extensions: ["zip"] }] }); if (!path || Array.isArray(path)) return; await settingsService.importApplicationData(path); notify("Дані перевірено, оновлено до поточної версії та відновлено.", "success"); window.location.reload(); } catch (error) { notify(typeof error === "string" ? error : error instanceof Error ? error.message : "Не вдалося імпортувати архів даних.", "error"); } };
  ...
  return <PageFrame header={<PageTitle title="Налаштування" subtitle="Підписанти, Excel-база та перенесення даних" />} className="settings-page">
    ...
    {excelOpen && <Modal title="Імпорт Excel-бази" onClose={() => { setExcelOpen(false); setExcelImportError(""); setExcelImportResult(null); }} className="personnel-import-modal"><div className="personnel-import-modal__body">
      <p>Оберіть, як застосувати дані з локального Excel-файлу.</p>
      {excelImportError && <div className="excel-import-error" role="alert"><b>Імпорт зупинено</b><span>{excelImportError}</span><button className="button" type="button" onClick={() => void navigator.clipboard.writeText(excelImportError).then(() => notify("Текст помилки скопійовано.", "success"))}><ClipboardCopy />Копіювати помилку</button></div>}
