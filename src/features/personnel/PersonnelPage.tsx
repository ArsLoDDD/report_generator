...
import { AlertCircle, Car, ClipboardCheck, List, PackageOpen, Pencil, RefreshCw, Trash2, UserPlus, Users, X } from "lucide-react";
...
export function PersonnelPage({ people, totalCount, hasMore, isLoading, isLoadingMore, errorMessage, onCreate, onUpdate, onDelete, onRefresh, onLoadMore }: PersonnelPageProps) {
  const [pageTab, setPageTab] = useState<"registry" | "control">(() => !isSimpleEdition && localStorage.getItem("personnel-page-tab") === "control" ? "control" : "registry");
  ...
  const tools = <div className="personnel-page-tools">{pageTabs}<div className="table-tools main-tools"><SearchInput placeholder="Пошук за ПІБ, ІПН або посадою…" value={query} onChange={setQuery} /><FilterButton active={filtersOpen} onClick={() => setFiltersOpen(true)} label="Додаткові фільтри" /></div></div>;
  ...
  if (pageTab === "control" && !isSimpleEdition) return <PageFrame header={<PageTitle title="Особовий склад" subtitle="Єдиний контроль актуального місця перебування та зв’язок із БЧС" />} tools={<div className="personnel-page-tools">{pageTabs}</div>} className="personnel-page personnel-page--control"><PersonnelControl people={people} hasMorePeople={hasMore} onLoadMorePeople={onLoadMore} /></PageFrame>;
