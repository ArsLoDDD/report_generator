...
export function PersonnelPage({ people, totalCount, hasMore, isLoading, isLoadingMore, errorMessage, onCreate, onUpdate, onDelete, onRefresh, onLoadMore }: PersonnelPageProps) {
  ...
  const importExcel = async () => { try { const path = await personnelService.selectExcelFile(); if (!path) return; const count = await personnelService.importExcel(path); await onRefresh(); notify(`Імпортовано записів: ${count}.`, "success"); } catch (error) { notify(errorText(error), "error"); } };
  const exportExcel = async () => { try { const path = await personnelService.exportExcel(); if (path) notify("Excel-файл успішно експортовано.", "success"); } catch (error) { notify(errorText(error), "error"); } };
