...
describe("Tauri command contracts", () => {
  ...
  it("sends settings, templates, generation and reports commands exactly", () => {
    settingsService.get(); settingsService.updateSigner("main", { fullName: "Іваненко Іван", rank: "майор", position: "командир" }); settingsService.updateVisiblePersonnelColumns(["rank"]); settingsService.openApplicationDirectory(); settingsService.createDatabaseBackup(); settingsService.getUpdateStatus(); settingsService.inspectOfflineUpdate("/tmp/update.shupd"); settingsService.installOfflineUpdate("/tmp/update.shupd"); settingsService.exportApplicationData("/tmp/backup.zip", { database: true, settings: true, customVariables: true, templates: true, reports: false }); settingsService.importApplicationData("/tmp/backup.zip");
    ...
    expect(invoke.mock.calls).toEqual(expect.arrayContaining([
      ["get_app_settings"], ["update_signer_settings", { role: "main", signer: { fullName: "Іваненко Іван", rank: "майор", position: "командир" } }], ["update_visible_personnel_columns", { columns: ["rank"] }], ["open_application_directory"], ["create_database_backup"], ["get_update_status"], ["inspect_offline_update", { path: "/tmp/update.shupd" }], ["install_offline_update", { path: "/tmp/update.shupd" }], ["export_application_data", { path: "/tmp/backup.zip", options: { database: true, settings: true, customVariables: true, templates: true, reports: false } }], ["import_application_data", { path: "/tmp/backup.zip" }],
