...
describe("Tauri command contracts", () => {
  ...
  it("sends settings, templates, generation and reports commands exactly", () => {
    ...
    expect(invoke.mock.calls).toEqual(expect.arrayContaining([
      ...
      ["select_template_file"], ["validate_template", { templatePath: "/templates/a.docx", personnelIds: [1], vehicleIds: [], parameters: { дата_рапорту: "2026-08-11" } }], ["generate_report", { request: { templatePath: "/templates/a.docx", personnelIds: [1], parameters: { дата_рапорту: "2026-08-11" } } }], ["open_generated_report", { reportPath: "/reports/a.docx" }], ["open_generated_report_folder", { folderPath: "/reports" }],
