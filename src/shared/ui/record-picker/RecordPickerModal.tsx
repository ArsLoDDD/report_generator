...
export function RecordPickerModal({ title, items, selectedIds, onToggle, onClose, multiple = true }: Props) {
  const [query, setQuery] = useState("");
  const filtered = useMemo(() => items.filter((item) => includesSearch(query, item.title, item.subtitle, item.owner ?? "")), [items, query]);
  return <Modal title={title} onClose={onClose} className="crew-member-picker"><div className="crew-member-picker__body"><SearchInput placeholder="Пошук за ПІБ, званням або посадою…" value={query} onChange={setQuery} /><div className="crew-member-picker__list">{filtered.map((item) => <label key={item.id} className={selectedIds.includes(item.id) ? "selected" : ""}><input type={multiple ? "checkbox" : "radio"} checked={selectedIds.includes(item.id)} onChange={() => onToggle(item.id)} /><span><b>{item.title}</b><small>{item.subtitle}</small>{item.owner && <em>{item.owner}</em>}</span></label>)}</div></div><footer className="modal-actions"><button className="button primary" onClick={onClose}>Готово</button></footer></Modal>;
}
