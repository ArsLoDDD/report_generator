...
export default function App() {
  ...
  return <div className="product-shell"><aside className="sidebar"><div className="product-logo"><FileCheck2 /><div><b>Генератор рапортів</b><span>по шаблону</span></div></div><nav>{navigation.map(([id, label, Icon]) => <button key={id} onClick={() => setScreen(id)} className={screen === id ? "nav-active" : ""}><Icon size={23} />{label}</button>)}</nav><div className="version">Версія 1.0.0</div></aside><main className="workspace"><div className="window-buttons"><span>−</span><span>□</span><span>×</span></div>{screen === "generator" && <ReportGenerationPage template={selectedTemplate} people={people} selected={selectedPeople} onToggle={togglePerson} onAll={toggleAllPeople} onChoose={setSelectedTemplate} />}{screen === "templates" && <TemplatesPage selected={templateInfo} onSelect={setTemplateInfo} />}{screen === "people" && <PersonnelPage people={people} detailsOpen={detailsOpen} onDetails={() => setDetailsOpen((value) => !value)} />}{screen === "generated" && <GeneratedReportsPage />}{screen === "settings" && <SettingsPage active={settingsTab} onChange={setSettingsTab} />}</main></div>;
}
