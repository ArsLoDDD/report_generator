...
vi.mock("./shared/services/personnelService", () => ({
  ...
  ], totalCount: 2 }), create: vi.fn(), update: vi.fn(), delete: vi.fn(), listCustomFields: vi.fn().mockResolvedValue([]), listPersonnelFields: vi.fn().mockResolvedValue([]), createCustomField: vi.fn(), listVehicleCustomFields: vi.fn().mockResolvedValue([]), createVehicleCustomField: vi.fn(), updateVehicleCustomField: vi.fn(), deleteVehicleCustomField: vi.fn() }
}));
...
describe("navigation and report generation", () => {
  it("opens every primary workspace from the sidebar", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "БпЛА та БпАК" }));
