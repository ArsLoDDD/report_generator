...
export function AppSidebar({ screen, collapsed, warnings, onNavigate, onToggleCollapsed }: Props) {
  ...
  return <aside className="sidebar">
    ...
    <section className="sidebar-middle">{warnings.length > 0 && <section className="sidebar-warnings" aria-label="Попередження програми">{warnings.map((warning) => <article key={warning.code} title={warning.message}><AlertTriangle /><div><b>{warning.title}</b><span>{warning.message}</span></div></article>)}</section>}</section>
